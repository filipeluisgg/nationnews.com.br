...
describe("PATCH /api/v1/users/[username]", () => {
	describe("Anonymous user", () => {
		...
		test("With duplicated 'email'", async () => {
			await orchestrator.createUser({ email: "email1@curso.dev" });

			const createdUser2 = await orchestrator.createUser({ email: "email2@curso.dev" });

			const response = await fetch(`http://localhost:3000/api/v1/users/${createdUser2.username}`, {
				method: "PATCH",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ email: "email1@curso.dev" }),
			});
		...
		test("With unique 'email'", async () => {
			...
			const response = await fetch(`http://localhost:3000/api/v1/users/${createdUser.username}`, {
				method: "PATCH",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ email: "uniqueEmail2@curso.dev" }),
			});
